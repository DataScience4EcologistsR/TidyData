...
dt_treatment<- dt %>% group_by(treatment) %>% summarize(mean_height= mean(height))
dt_treatment

# Maybe it would make more sense sense to take look at the max per species per year: 
dt_treatment_sp<- dt %>% group_by(treatment,year) %>% summarize(mean_height= mean(height), max_hieght=max(height))
dt_treatment_sp
